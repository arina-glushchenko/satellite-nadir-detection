                 ...
                 detect_result_group_t *group, int* dims)
{
    ...
    // Check for single output mode (input1 and input2 NULL, assuming single concatenated output)
    if (input1 == NULL && input2 == NULL && input0 != NULL) {
        printf("Using single output mode (concatenated grids, no classes, class 0 assumed)\n");
        // Assume positions: 1024 (stride8) + 256 (16) + 64 (32) = 1344
        // Each position: 5 channels (tx,ty,tw,th,obj)
        // Use first anchor per scale
        int pos_offset[3] = {0, 1024, 1280};
        int grid_sizes[3] = {32, 16, 8};
        int strides[3] = {8, 16, 32};
        int *anchors[3] = {(int*)anchor0, (int*)anchor1, (int*)anchor2};
        int8_t thres_i8 = qnt_f32_to_affine(conf_threshold, qnt_zps[0], qnt_scales[0]);
        for (int s = 0; s < 3; s++) {
            int grid_h = grid_sizes[s];
            int grid_w = grid_sizes[s];
            int stride = strides[s];
            int num_pos = grid_h * grid_w;
            for (int h = 0; h < grid_h; h++) {
                for (int w = 0; w < grid_w; w++) {
                    int p = pos_offset[s] + h * grid_w + w;
                    int8_t *hw_ptr = input0 + p * 5;
                    int8_t box_confidence = hw_ptr[4];
                    if (box_confidence >= thres_i8) {
                        float box_conf_f32 = deqnt_affine_to_f32(box_confidence, qnt_zps[0], qnt_scales[0]);
                        float limit_score = box_conf_f32 * 1.0f;  // class_prob = 1.0
                        if (limit_score > conf_threshold) {
                            float box_x = deqnt_affine_to_f32(hw_ptr[0], qnt_zps[0], qnt_scales[0]) * 2.0f - 0.5f + w;
                            float box_y = deqnt_affine_to_f32(hw_ptr[1], qnt_zps[0], qnt_scales[0]) * 2.0f - 0.5f + h;
                            float box_w = deqnt_affine_to_f32(hw_ptr[2], qnt_zps[0], qnt_scales[0]) * 2.0f;
                            float box_h = deqnt_affine_to_f32(hw_ptr[3], qnt_zps[0], qnt_scales[0]) * 2.0f;
                            box_w = box_w * box_w * (float)anchors[s][0];  // first anchor
                            box_h = box_h * box_h * (float)anchors[s][1];
                            box_x *= (float)stride;
                            box_y *= (float)stride;
                            box_x -= box_w / 2.0f;
                            box_y -= box_h / 2.0f;
                            ...
                            objProbs.push_back(limit_score);
                            classId.push_back(0);  // default class 0
                            validCount++;
                        }
                    }
                }
